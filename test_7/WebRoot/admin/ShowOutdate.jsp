<%@page import="admin.Notice_admin.ShowOutdate"%>
...
<body onLoad="div_page(${currentPage},${TotalPage },&quot;ShowOutdate&quot;)">
	...
	var string;
	var user_idtest;
	<%	 ArrayList<ShowOutdate> arrayList=(ArrayList<ShowOutdate>)request.getAttribute("ShowOutdate_list"); 
		int size=arrayList.size();
		  ShowOutdate user=null;
		...
		request.setAttribute("user", user);
		 ...
		 string="<tr><td >资产编号</td><td > ${user.asset_id}</td></tr>"+
		 					"<tr><td>设备名称</td><td> ${user.dev_name}</td></tr>"+
							"<tr><td>保修终止时间</td><td>${user.maint_end_time}</td></tr>"+
							"<tr><td>代维单位名称</td><td>${user.daiWei_unit}</td></tr>"+
							"<tr><td>邮政编码</td><td>${user.postcode}</td></tr>"+
							"<tr><td>单位负责人</td><td>${user.unit_res}</td></tr>"+
								"<tr><td>白天固定电话</td><td>   ${user.tel_day}</td></tr>"+
							"<tr><td>夜间固定电话</td><td>${user.tel_night}</td></tr>"+
							"<tr><td>移动电话</td><td>${user.mol_tel}</td></tr>"+
							"<tr><td>Email地址</td><td>${user.email}</td></tr>"+
							"<tr><td>代维单位地址</td><td>${user.address}</td></tr>"+
							"<tr><td>代维主管单位</td><td>${user.dai_controller}</td></tr>";
	...
	<div id="content">
		...
		<div id="content-header">
			...
			<a href="<%=basePath%>user_servlet.do?currentPage=1&oper=search" title="Go to Home" class="tip-bottom"><i
					class="icon-home"></i> Home</a> <a>到期资产动态</a>
			</c:when>
			<c:otherwise>
									  ...
									  <tr >
									        ...
									        <span class="icon" >
									         <i class="icon-th" ></i>
									          ...
									          <h6  class="table_title_first_name ">到期资产动态</h6>
...
<ul id="barconSimple"></ul></div>								          
					...
					<tbody>
						<c:forEach items="${ShowOutdate_list }" var="list">
							<tr>
								<td>${list.asset_id }</td>
								<td>${list.address }</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<table  class="table table-bordered table-striped" style="margin-left:auto; margin-right:auto; " id="delSimple">
