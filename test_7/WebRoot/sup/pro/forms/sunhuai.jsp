		...
		//显示分页栏部分
		    ...
		    var tempStrSimple;//顶部页码
				if(currentPage==1){
		    		...
		    		tempStr = "<li><a href=\"\\test_7\\forms_sunhuai.do?oper=search&city=${city}&currentPage="+(currentPage-1)+"\""+" >&laquo;</a></li>";
		    		tempStrSimple = "<li><a href=\"\\test_7\\forms_sunhuai.do?oper=search&city=${city}&currentPage="+(currentPage-1)+"\""+" >&laquo;</a></li>";
		    		//这里也要改，改成相应的servlet
		   		}
		    	for(var i=1;i<=totalPage;i++){
		    		if(currentPage==i){
		    			...
		    			tempStr+="<li><a href=\"\\test_7\\forms_sunhuai.do?oper=search&city=${city}&currentPage="+i+"\">"+i+"</a></li>";
		    		}
		    		console.log(i);
		    	...
		    	if(currentPage==totalPage||totalPage==0){
		    		...
		    		tempStr+= "<li><a href=\"\\test_7\\forms_sunhuai.do?oper=search&city=${city}&currentPage="+(currentPage+1)+"\""+" >&raquo;</a></li>";
		    		tempStrSimple+="<li><a href=\"\\test_7\\forms_sunhuai.do?oper=search&city=${city}&currentPage="+(currentPage+1)+"\""+" >&raquo;</a></li>";
		    	}
		...
		<div id="content-header">
			<div id="breadcrumb">
				<a href="<%=basePath %>pro_sup_servlet.do?currentPage=1&oper=search" title="Go to Home" class="tip-bottom"><i
					class="icon-home"></i> Home</a> <a>资产损坏率报表</a>
			</div>
       <form action="<%=basePath %>forms_sunhuai.do" method="get"   onsubmit="return chkfrm(this)">
		...
		<tr><td style="width:280px"></td><td style="width:280px"><span style=" color:red " id="start"></td><td style="width:280px"><span style=" color:red " id="end">  </span></td></tr>
					...
					<input name="generate" class="btn btn-primary "
					 ...
					 <a  class="btn btn-primary" style="color:white" href="<%=basePath %>Export_Servlet?startTime=${startTime}&endTime=${endTime}&city=${city}&currentPage=${currentPage}&oper=sunhuai">
									  ...
									  <tr >
									        ...
									        <span class="icon" >
									         <i class="icon-th" ></i>
									          ...
									          <h6  class="table_title_first_name ">资产损坏率报表</h6>
...
<ul id="barconSimple">
									...
									<tbody>
										<c:forEach items="${sunhuai_list}" var="list">
										<tr>
											<td>${list.asset_type }</td>
											<td>${list.sunhuai_num }</td>
											<td>${list.asset_status_sum }</td>
											<td>${list.sunhuai_rate }%</td>
										</tr>
										</c:forEach>	
									</tbody>
								</table>
